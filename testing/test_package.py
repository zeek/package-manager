...
@pytest.mark.parametrize("value", ["builtin", "directory"])
def test_package_status_maps_legacy_non_git_methods_to_none(value: str) -> None:
    # Non-Git packages have no tracking method; older manifests use string
    # values for them that must round-trip to None.
    status = PackageStatus(tracking_method=value)
    assert status.tracking_method is None

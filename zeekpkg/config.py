...
class Config(configparser.ConfigParser):
    """This class is a minor ConfigParser specialization to store the various
    config settings needed by zkg. Like ConfigParser it supports loading from
    and saving to ini-style config files, but also defaults to a config file
    location, supports overriding values from command-line arguments, and
    provides convenience methods for accessing specific settings.
    """
    ...
    def reset(self, args: argparse.Namespace | None = None) -> None:
        """Resets the configuration to a bare-bones, built-in default."""
    ...
    def sanity_check(self) -> None:
        """Checks the settings for acceptable values and exits upon violations."""
    ...
    def update_from_file(self, configfile: str) -> None:
        """Loads the given config file, adding its content to the current settings.

        This also applies canonicalization and sanity-checking.
        """
        if not configfile:
            return
    ...
    def update_from_args(self, args: argparse.Namespace) -> None:
        """Updates the current settings as per the given command-line arguments.

        This currently only supports providing additional package sources.  Also
        applies canonicalization and sanity-checking.
        """
    ...
    def zeek_dist(self) -> str | None:
        ...
        on the system, in which case this returns None.
        """
    ...
    def sources_clone_dir(self) -> str:
        ...
        """
        return os.path.join(self.state_dir(), "clones", "source")
    ...
    def packages_clone_dir(self) -> str:
        ...
        """
        return os.path.join(self.state_dir(), "clones", "package")
    ...
    def default_config_dir(self) -> str:
        ...
        return os.getenv("ZEEK_ZKG_CONFIG_DIR") or self.home_config_dir()

    def default_state_dir(self) -> str:
        """Default directory in which zkg stores all of its state."""
        return os.getenv("ZEEK_ZKG_STATE_DIR") or self.home_config_dir()
